...
export default function SpatialTab() {
  const sources = [
    ...
    { label: 'FX', x: 22, y: 38, colorGlow: 'shadow-[0_0_20px_rgba(255,215,0,0.8)]', bg: 'bg-yellow-500' },
  ]
  ...
  return (
    <div className="h-full flex flex-col space-y-6 animate-in fade-in duration-500">
      <div className="crystal-glass flex-1 rounded-2xl relative overflow-hidden flex flex-col items-center justify-center p-8 shadow-2xl">
        ...
        <div className="w-[min(450px,80vw)] aspect-square rounded-full crystal-bubble relative shadow-[0_0_60px_rgba(0,229,255,0.1),inset_0_0_40px_rgba(255,255,255,0.05)] flex items-center justify-center border-t border-b border-white/20">
          ...
          {sources.map((source) => (
            <div key={source.label} className={`absolute w-8 h-8 -ml-4 -mt-4 rounded-full crystal-glass flex items-center justify-center ${source.colorGlow}`} style={{ left: `${source.x}%`, top: `${source.y}%` }}>
      ...
      <div className="h-[200px] shrink-0 crystal-glass rounded-2xl p-6 flex flex-col relative shadow-[0_15px_40px_rgba(0,0,0,0.6)]">
        ...
        <div className="flex-1 flex flex-col justify-around px-8">
          {sliders.map((slider) => (
            <div key={slider.label} className="flex items-center gap-6 group">
              <span className="w-32 text-right text-[10px] font-bold tracking-[0.15em] text-white/60 group-hover:text-white transition-colors">{slider.label}</span>
              <div className="flex-1 h-3 crystal-bubble rounded-full relative flex items-center">
                <div className="absolute left-1 right-1 h-1 bg-black/60 rounded-full" />
                <div className="absolute left-1 h-1 rounded-full opacity-80 shadow-[0_0_10px_currentColor] transition-all duration-300" style={{ width: `calc(${slider.value}% - 8px)`, backgroundColor: slider.color, color: slider.color }} />
                <div className="absolute w-5 h-5 crystal-glass rounded-full shadow-[0_0_10px_rgba(255,255,255,0.4)]" style={{ left: `calc(${slider.value}% - 10px)` }} />
              </div>
              <span className="w-12 text-left text-[10px] font-mono font-bold text-white" style={{ color: slider.color }}>{slider.value}%</span>
            </div>
          ))}
        </div>
      </div>
